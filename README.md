...
Для обеспечения высокой доступности и минимальных задержек система разделена на следующие функциональные домены:

1. **Основной домен (user.hh.ru)**:
   - Пользовательские функции: регистрация, авторизация, просмотр лент
   - Обрабатывает 92.44% трафика (основная аудитория из России)

2. **HR домен (hr.hh.ru)**:
   - Функции для рекрутеров: размещение вакансий, управление откликами
   - Отдельный домен для защиты от DDoS и изоляции критичного функционала

3. **API домен (api.hh.ru)**:
   - Общее API для мобильных приложений и партнеров
   - Реализует REST и GraphQL интерфейсы

#### Обоснование расположения ДЦ
На основе анализа географического распределения аудитории (раздел 1) предлагается следующая схема:

| Локация ДЦ | Зоны покрытия | Кол-во серверов | Задержка | Комментарий |
|------------|---------------|-----------------|----------|-------------|
| Москва | Центральная Россия | 200 | <50 мс | Основной ДЦ для 92.44% трафика |
| Екатеринбург | Урал, Сибирь | 50 | <70 мс | Региональный ДЦ для снижения задержек |
| Алма-Ата | Казахстан | 30 | <40 мс | Локальный ДЦ для 0.44% аудитории |
| Минск | Беларусь | 20 | <30 мс | Обслуживает 1.8% трафика |

**Критерии выбора**:
- Минимизация задержек для ключевых рынков
- Соответствие требованиям 152-ФЗ (хранение данных российских пользователей в РФ)
- Стоимость эксплуатации: $ 0.02/ГБ в РФ против $ 0.05/ГБ за рубежом

#### Расчет распределения запросов по ДЦ
На основе данных из раздела 2 (2 887 RPS):

| Тип запроса | Москва | Екатеринбург | Алма-Ата | Минск | Всего |
|-------------|--------|--------------|----------|-------|-------|
| Просмотр лент | 1,200 | 200 | 50 | 50 | 1,500 |
| Поиск | 400 | 60 | 20 | 20 | 500 |
| CRUD резюме | 140 | 40 | 10 | 10 | 200 |
| Управление вакансиями | 45 | 5 | - | - | 50 |
| Авторизация | 80 | 10 | 5 | 5 | 100 |
| **Итого** | **1,865** | **315** | **85** | **85** | **2,887** |

**Примечания**:
- Запись данных (CRUD) только в московский ДЦ с последующей репликацией
- Чтение распределяется по ближайшему ДЦ
- Коэффициент репликации: 3 для России, 2 для СНГ

#### Схема DNS балансировки
1. **Географический DNS**:
   ``` dns
   ; Основные записи
   hh.ru.      IN A 195.88.54.1 (Москва)
   hh.ru.      IN A 195.88.54.2 (Екатеринбург)
   
   ; GeoDNS записи
   ru.hh.ru.   IN A 195.88.54.1
   kz.hh.ru.   IN A 195.88.55.1
   by.hh.ru.   IN A 195.88.56.1
   ```
...
1. **IP-адреса**:
   - API: 195.88.54.10 (анонсируется из всех ДЦ)
   - Поиск: 195.88.54.20

2. **Конфигурация BGP**:
   ```plaintext
   router bgp 12345
    neighbor 195.88.54.254 remote-as 12345
    network 195.88.54.0/24
    network 195.88.54.10/32
    network 195.88.54.20/32
   ```

#### Механизм регулировки трафика
1. **Алгоритм балансировки**:
   - По задержке (для API)
   - По загрузке CPU (для статики)

2. **Переключение при сбоях**:
   - Автоматический failover при недоступности ДЦ > 1 мин
   - Приоритеты маршрутизации:
     1. Локальный ДЦ
     2. Ближайший региональный
     3. Москва

3. **Мониторинг**:
   - Prometheus с алертами через Alertmanager
   - Grafana dashboard с ключевыми метриками

#### Summary
| Метрика | Значение                            |
|---------|-------------------------------------|
| Общая нагрузка | 2,887 RPS                           |
| Максимальная задержка | <100 мс для 95% пользователей       |
| Время восстановления | <5 мин при выходе ДЦ из строя       |
| Стоимость трафика | $ 12,000/мес (из расчета $ 0.02/ГБ) |
